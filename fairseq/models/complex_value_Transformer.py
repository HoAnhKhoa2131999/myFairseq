...
from fairseq.modules.Layers import EncoderLayer, DecoderLayer
...
def get_position(src_tokens):
    """Hàm để đánh số các phần tử khác không theo thứ tự"""
    B = src_tokens.ne(0)
    N = torch.sum(B,1)
    out= torch.zeros_like(src_tokens, dtype=torch.int64)

    for i in range(out.size(0)): # Chưa biết cách cải tiến
        num = int(N[i])
...
def get_non_pad_mask(seq, padding_idx):
    assert seq.dim() == 2
...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--kdim', type=int, metavar='N',
                            help='kdim')
        parser.add_argument('--vdim', type=int, metavar='N',
                            help='vdim')
        # fmt: on
...
class TransformerEncoder(FairseqEncoder):
    ...
    def build_encoder_layer(self, args):
        layer = EncoderLayer(args.encoder_embed_dim, args.encoder_ffn_embed_dim,args.encoder_attention_heads,args.kdim, args.vdim, args.dropout)
    ...
    def forward(
        ...
        # -- Prepare masks
        slf_attn_mask = get_attn_key_pad_mask(seq_k=src_tokens, seq_q=src_tokens, padding_idx = self.padding_idx)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = DecoderLayer(args.encoder_embed_dim, args.encoder_ffn_embed_dim,args.encoder_attention_heads,args.kdim, args.vdim, args.dropout)
        if getattr(args, "checkpoint_activations", False):
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
        return layer
    ...
    def extract_features_scriptable(
        ...
        slf_attn_mask_keypad = get_attn_key_pad_mask(seq_k=prev_output_tokens, seq_q=prev_output_tokens, padding_idx = self.padding_idx)
...
@register_model_architecture("complex_transformer", "complex_value_order_transformer")
def complex_value_order_transformer(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 2048)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.encoder_normalize_before = getattr(args, "encoder_normalize_before", False)
    ...
    args.decoder_attention_heads = getattr(args, "decoder_attention_heads", 8)
    args.dropout = getattr(args, "dropout", 0.1)
    args.kdim = getattr(args, "kdim", 64)
    args.vdim = getattr(args, "vdim", 64)
    base_architecture(args)
